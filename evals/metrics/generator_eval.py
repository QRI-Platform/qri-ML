...
async def eval_generator(inputs: dict, outputs: dict, reference_outputs: dict) -> dict:
    ...
    """
    try:
        expected_tool_call = reference_outputs.get("has_tool_call", False)
        actual_tool_call = outputs.get("has_tool_call", False)

        # Tool Call Accuracy Check
        if expected_tool_call != actual_tool_call:
            return EvaluationResult(
                key="generator_evaluation",
                score=0.0,
